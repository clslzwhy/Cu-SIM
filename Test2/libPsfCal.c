    ...
    mclOutputHandlerFcn print_handler)
{
    ...
    if (!mclmcrInitialize())
        return false;
    if (!GetModuleFileName(GetModuleHandle("libPsfCal"), path_to_dll, _MAX_PATH))
        return false;
    {
...
{
    return libPsfCalInitializeWithHandlers(mclDefaultErrorHandler, 
...
{
    return mclFeval(_mcr_inst, "libPsfCal", nlhs, plhs, nrhs, prhs);
}

LIB_libPsfCal_C_API 
bool MW_CALL_CONV mlfLibPsfCal(int nargout, mxArray** R1, mxArray** R2, mxArray** R3, 
                               mxArray** R4, mxArray** R5, mxArray** R6, mxArray** R7, 
                               mxArray** R8, mxArray** R9, mxArray* lambda, mxArray* 
                               sizeLoc)
{
    return mclMlfFeval(_mcr_inst, "libPsfCal", nargout, 9, 2, R1, R2, R3, R4, R5, R6, R7, R8, R9, lambda, sizeLoc);
}
