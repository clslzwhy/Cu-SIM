...
#define EXPORTING_libMatlab_edgetaper 1
    ...
    mclOutputHandlerFcn print_handler)
{
    ...
    if (!mclmcrInitialize())
        return false;
    if (!GetModuleFileName(GetModuleHandle("libMatlab_edgetaper"), path_to_dll, _MAX_PATH))
        return false;
    {
...
{
    return libMatlab_edgetaperInitializeWithHandlers(mclDefaultErrorHandler, 
...
{
    return mclFeval(_mcr_inst, "Mat_edgetaper", nlhs, plhs, nrhs, prhs);
}

LIB_libMatlab_edgetaper_C_API 
bool MW_CALL_CONV mlfMat_edgetaper(int nargout, mxArray** dst, mxArray* src)
{
    return mclMlfFeval(_mcr_inst, "Mat_edgetaper", nargout, 1, 1, dst, src);
}
